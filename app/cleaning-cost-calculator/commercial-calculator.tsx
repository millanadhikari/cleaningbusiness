...
export function CommercialCalculator() {
  ...
  return <main className={styles.page}>
    ...
    <div className="site-navigation"><header className="wrap header"><Logo /><nav aria-label="Main navigation"><Link href="/">Home</Link><ServiceMenu /><Link href="/officecleaning">Office cleaning</Link><a href="#rates">Rate guide</a><a href="#proposal">Proposal</a></nav><div className="nav-utilities"><a href="tel:+61401356937" className="nav-phone" aria-label="Call WeDo on 0401 356 937"><span className="phone-symbol"><Phone size={19} /></span><span><small>LET’S TALK CLEAN</small><strong>0401 356 937</strong></span></a><span className="nav-divider" /><Link href="/sign-in" className="nav-login" aria-label="Staff login" title="Staff login"><UserRound size={21} /></Link></div><button type="button" className="menu-toggle" aria-label={menu ? "Close navigation" : "Open navigation"} aria-expanded={menu} aria-controls="calculator-mobile-navigation" onClick={() => setMenu(!menu)}>{menu ? <X /> : <Menu />}</button></header>{menu && <nav id="calculator-mobile-navigation" className="mobile-menu" aria-label="Mobile navigation"><Link href="/" onClick={() => setMenu(false)}>Home</Link><ServiceMenu onNavigate={() => setMenu(false)} /><Link href="/officecleaning" onClick={() => setMenu(false)}>Office cleaning</Link><a href="#rates" onClick={() => setMenu(false)}>Rate guide</a><a href="#proposal" onClick={() => setMenu(false)}>Proposal</a><Link href="/sign-in" onClick={() => setMenu(false)}>Staff login</Link></nav>}</div>
